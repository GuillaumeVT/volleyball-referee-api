...
public class GameDao {
    ...
    private final static ProjectionOperation sGameSummaryProjection = Aggregation.project()
            .and(_id).as(_id)
            .and(Game.Fields.createdBy).as(GameSummary.Fields.createdBy)
            .and(Game.Fields.createdAt).as(GameSummary.Fields.createdAt)
            .and(Game.Fields.updatedAt).as(GameSummary.Fields.updatedAt)
            .and(Game.Fields.scheduledAt).as(GameSummary.Fields.scheduledAt)
            .and(Game.Fields.refereedBy).as(GameSummary.Fields.refereedBy)
            .and(Game.Fields.refereeName).as(GameSummary.Fields.refereeName)
            .and(Game.Fields.kind).as(GameSummary.Fields.kind)
            .and(Game.Fields.gender).as(GameSummary.Fields.gender)
            .and(Game.Fields.usage).as(GameSummary.Fields.usage)
            .and(Game.Fields.status).as(GameSummary.Fields.status)
            .and(Game.Fields.indexed).as(GameSummary.Fields.indexed)
            .and(Game.Fields.league + "." + _id).as(GameSummary.Fields.leagueId)
            .and(Game.Fields.league + "." + LeagueSummary.Fields.name).as(GameSummary.Fields.leagueName)
            .and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).as(GameSummary.Fields.divisionName)
            .and(Game.Fields.homeTeam + "." + _id).as(GameSummary.Fields.homeTeamId)
            .and(Game.Fields.homeTeam + "." + Team.Fields.name).as(GameSummary.Fields.homeTeamName)
            .and(Game.Fields.guestTeam + "." + _id).as(GameSummary.Fields.guestTeamId)
            .and(Game.Fields.guestTeam + "." + Team.Fields.name).as(GameSummary.Fields.guestTeamName)
            .and(Game.Fields.homeSets).as(GameSummary.Fields.homeSets)
            .and(Game.Fields.guestSets).as(GameSummary.Fields.guestSets)
            .and(Game.Fields.rules + "." + _id).as(GameSummary.Fields.rulesId)
            .and(Game.Fields.rules + "." + Rules.Fields.name).as(GameSummary.Fields.rulesName)
            .and(Game.Fields.score).as(GameSummary.Fields.score)
            .and(Game.Fields.referee1).as(GameSummary.Fields.referee1Name)
            .and(Game.Fields.referee2).as(GameSummary.Fields.referee2Name)
            .and(Game.Fields.scorer).as(GameSummary.Fields.scorerName);
    ...
    public Page<GameSummary> listLiveGames(List<GameType> kinds, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.status).is(GameStatus.LIVE).and(Game.Fields.indexed).is(true).and(Game.Fields.kind).in(kinds).and(Game.Fields.gender).in(genders);
    ...
    public Page<GameSummary> listGamesMatchingToken(String token, List<GameStatus> statuses, List<GameType> kinds, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.indexed).is(true).and(Game.Fields.status).in(statuses).and(Game.Fields.kind).in(kinds).and(Game.Fields.gender).in(genders).orOperator(
                Criteria.where(Game.Fields.homeTeam + "." + Team.Fields.name).regex(".*" + token + ".*", "i"),
    ...
    public Page<GameSummary> listGamesWithScheduleDate(LocalDate date, List<GameStatus> statuses, List<GameType> kinds, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.indexed).is(true).and(Game.Fields.status).in(statuses).and(Game.Fields.kind).in(kinds).and(Game.Fields.gender).in(genders)
    ...
    public List<GameSummary> listGamesInDivision(UUID leagueId, String divisionName) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sGameSummaryProjection, sortOperation),
                mongoTemplate.getCollectionName(Game.class), GameSummary.class).getMappedResults();
    }
    ...
    public Page<GameSummary> listGamesInLeague(UUID leagueId, List<GameStatus> statuses, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).in(statuses).and(Game.Fields.gender).in(genders);
    ...
    public Page<GameSummary> listGamesInDivision(UUID leagueId, String divisionName, List<GameStatus> statuses, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).in(statuses).and(Game.Fields.gender).in(genders);
    ...
    public Page<GameSummary> listGamesOfTeamInLeague(UUID leagueId, UUID teamId, List<GameStatus> statuses, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).in(statuses)
                .orOperator(Criteria.where(Game.Fields.homeTeam + "." + _id).is(teamId), Criteria.where(Game.Fields.guestTeam + "." + _id).is(teamId));
    ...
    public LeagueDashboard findGamesInLeagueGroupedByStatus(UUID leagueId) {
        ...
        FacetOperation facetOperation = Aggregation
                .facet(liveMatchOperation, sGameSummaryProjection, sortOperation).as(LeagueDashboard.Fields.liveGames)
                .and(last10MatchOperation, sGameSummaryProjection, sortOperation, limitOperation).as(LeagueDashboard.Fields.last10Games)
                .and(next10MatchOperation, sGameSummaryProjection, sortOperation, limitOperation).as(LeagueDashboard.Fields.next10Games);
    ...
    public List<GameSummary> listLiveGamesInLeague(UUID leagueId) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).is(GameStatus.LIVE));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sGameSummaryProjection, sortOperation),
                mongoTemplate.getCollectionName(Game.class), GameSummary.class).getMappedResults();
    }
    ...
    public List<GameSummary> listLast10GamesInLeague(UUID leagueId) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).is(GameStatus.COMPLETED));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
        LimitOperation limitOperation = Aggregation.limit(10L);
    ...
    public List<GameSummary> listNext10GamesInLeague(UUID leagueId) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).is(GameStatus.SCHEDULED));
    ...
    public Page<GameSummary> listGamesOfTeamInDivision(UUID leagueId, String divisionName, UUID teamId, List<GameStatus> statuses, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).in(statuses)
                .orOperator(Criteria.where(Game.Fields.homeTeam + "." + _id).is(teamId), Criteria.where(Game.Fields.guestTeam + "." + _id).is(teamId));
    ...
    public LeagueDashboard findGamesInDivisionGroupedByStatus(UUID leagueId, String divisionName) {
        MatchOperation divisionMatchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName));
        MatchOperation liveMatchOperation = Aggregation.match(Criteria.where(Game.Fields.status).is(GameStatus.LIVE));
        MatchOperation last10MatchOperation = Aggregation.match(Criteria.where(Game.Fields.status).is(GameStatus.COMPLETED));
        MatchOperation next10MatchOperation = Aggregation.match(Criteria.where(Game.Fields.status).is(GameStatus.SCHEDULED));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
    ...
    public List<GameSummary> listLiveGamesInDivision(UUID leagueId, String divisionName) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).is(GameStatus.LIVE));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sGameSummaryProjection, sortOperation),
                mongoTemplate.getCollectionName(Game.class), GameSummary.class).getMappedResults();
    }
    ...
    public List<GameSummary> listLast10GamesInDivision(UUID leagueId, String divisionName) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).is(GameStatus.COMPLETED));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.DESC, GameSummary.Fields.scheduledAt);
        LimitOperation limitOperation = Aggregation.limit(10L);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sGameSummaryProjection, sortOperation, limitOperation),
                mongoTemplate.getCollectionName(Game.class), GameSummary.class).getMappedResults();
    ...
    public List<GameSummary> listNext10GamesInDivision(UUID leagueId, String divisionName) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).is(GameStatus.SCHEDULED));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.ASC, GameSummary.Fields.scheduledAt);
        LimitOperation limitOperation = Aggregation.limit(10L);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sGameSummaryProjection, sortOperation, limitOperation),
                mongoTemplate.getCollectionName(Game.class), GameSummary.class).getMappedResults();
    ...
    public Page<GameSummary> listGames(String userId, List<GameStatus> statuses, List<GameType> kinds, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.createdBy).is(userId).and(Game.Fields.status).in(statuses).and(Game.Fields.kind).in(kinds).and(Game.Fields.gender).in(genders);
    ...
    public List<GameSummary> listAvailableGames(String userId) {
        MatchOperation matchOperation = Aggregation.match(Criteria
                .where(Game.Fields.status).in(GameStatus.SCHEDULED, GameStatus.LIVE)
    ...
    public Page<GameSummary> listCompletedGames(String userId, Pageable pageable) {
        Criteria criteria = Criteria.where(Game.Fields.status).is(GameStatus.COMPLETED).orOperator(
                Criteria.where(Game.Fields.createdBy).is(userId), Criteria.where(Game.Fields.refereedBy).is(userId));
    ...
    public Page<GameSummary> listGamesInLeague(String userId, UUID leagueId, List<GameStatus> statuses, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Game.Fields.createdBy).is(userId).and(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.status).in(statuses).and(Game.Fields.gender).in(genders);
    ...
    public List<String> listDivisionsInLeague(String userId, UUID leagueId) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.createdBy).is(userId).and(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).exists(true).ne(""));
        ProjectionOperation projectionOperation = Aggregation.project().and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).as(GameSummary.Fields.divisionName);
        GroupOperation groupOperation = Aggregation
                .group(GameSummary.Fields.divisionName)
                .first(GameSummary.Fields.divisionName).as(GameSummary.Fields.divisionName);
    ...
    public List<GameScore> findByLeague_IdAndLeague_DivisionAndStatusOrderByScheduledAtAsc(UUID leagueId, String divisionName, GameStatus status) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Game.Fields.league + "." + _id).is(leagueId).and(Game.Fields.league + "." + Game.SelectedLeague.Fields.division).is(divisionName).and(Game.Fields.status).is(status));

        ProjectionOperation projectionOperation = Aggregation.project()
                .and(_id).as(_id)
                .and(GameSummary.Fields.scheduledAt).as(GameScore.Fields.scheduledAt)
                .and(Game.Fields.homeTeam + "." + Team.Fields.name).as(GameScore.Fields.homeTeamName)
                .and(Game.Fields.guestTeam + "." + Team.Fields.name).as(GameScore.Fields.guestTeamName)
                .and(Game.Fields.homeTeam + "." + Team.Fields.color).as(GameScore.Fields.homeTeamColor)
                .and(Game.Fields.guestTeam + "." + Team.Fields.color).as(GameScore.Fields.guestTeamColor)
                .and(Game.Fields.homeSets).as(GameScore.Fields.homeSets)
                .and(Game.Fields.guestSets).as(GameScore.Fields.guestSets)
                .and(Game.Fields.sets + "." + Set.Fields.homePoints).as(GameScore.Fields.sets + "." + SetSummary.Fields.homePoints)
                .and(Game.Fields.sets + "." + Set.Fields.guestPoints).as(GameScore.Fields.sets + "." + SetSummary.Fields.guestPoints);
        ...
        Query query = Query.query(
                Criteria
                        ...
                        .andOperator(new Criteria().orOperator(
                                Criteria.where(Game.Fields.createdBy).is(userId),
    ...
    public Optional<Game> findByIdAndAllowedUserAndStatus(UUID id, String userId, GameStatus status) {
        Query query = Query.query(
                Criteria
                        .where(_id).is(id)
                        .and(Game.Fields.status).is(status)
                        .andOperator(new Criteria().orOperator(
                                Criteria.where(Game.Fields.createdBy).is(userId),
                                Criteria.where(Game.Fields.refereedBy).is(userId))));
        return Optional.ofNullable(mongoTemplate.findOne(query, Game.class));
    ...
    public Optional<Game> findByIdAndAllowedUserAndStatusNot(UUID id, String userId, GameStatus status) {
        Query query = Query.query(
                Criteria
                        .where(_id).is(id)
                        .and(Game.Fields.status).ne(status)
                        .andOperator(new Criteria().orOperator(
                                Criteria.where(Game.Fields.createdBy).is(userId),
                                Criteria.where(Game.Fields.refereedBy).is(userId))));
        return Optional.ofNullable(mongoTemplate.findOne(query, Game.class));
    ...
    public boolean existsByCreatedByAndRules_IdAndStatus(String userId, UUID rulesId, GameStatus status) {
        Query query = Query.query(Criteria.where(Game.Fields.createdBy).is(userId).and(Game.Fields.status).is(status).and(Game.Fields.rules + "." + _id).is(rulesId));
        return mongoTemplate.exists(query, Game.class);
    }
    ...
    public boolean existsByCreatedByAndTeamAndStatus(String userId, UUID teamId, GameStatus status) {
        Query query = Query.query(
                Criteria
                        .where(Game.Fields.createdBy).is(userId)
                        .and(Game.Fields.status).is(status)
                        .andOperator(new Criteria().orOperator(
                                Criteria.where(Game.Fields.homeTeam + "." + _id).is(teamId),
    ...
    public boolean existsByCreatedByAndLeague_IdAndStatus(String userId, UUID leagueId, GameStatus status) {
        Query query = Query.query(Criteria.where(Game.Fields.createdBy).is(userId).and(Game.Fields.status).is(status).and(Game.Fields.league + "." + _id).is(leagueId));
        return mongoTemplate.exists(query, Game.class);
    }
    ...
    public long countByAllowedUserAndStatusNot(String userId, GameStatus status) {
        Query query = Query.query(
                Criteria
                        .where(Game.Fields.status).ne(status)
                        .andOperator(new Criteria().orOperator(
                                Criteria.where(Game.Fields.createdBy).is(userId),
                                Criteria.where(Game.Fields.refereedBy).is(userId))));
        return mongoTemplate.count(query, Game.class);
    ...
    public boolean updateReferee(UUID id, String refereedBy, String refereeName, long updatedAt) {
        ...
        Update update = new Update().set(Game.Fields.refereedBy, refereedBy).set(Game.Fields.refereeName, refereeName).set(Game.Fields.updatedAt, updatedAt);
