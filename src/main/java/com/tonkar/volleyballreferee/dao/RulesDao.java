...
public class RulesDao {
    ...
    private final static ProjectionOperation sRulesSummaryProjection = Aggregation.project()
            .and(_id).as(_id)
            .and(Rules.Fields.createdBy).as(RulesSummary.Fields.createdBy)
            .and(Rules.Fields.createdAt).as(RulesSummary.Fields.createdAt)
            .and(Rules.Fields.updatedAt).as(RulesSummary.Fields.updatedAt)
            .and(Rules.Fields.name).as(RulesSummary.Fields.name)
            .and(Rules.Fields.kind).as(RulesSummary.Fields.kind);
    ...
    public List<RulesSummary> listRules(String userId, List<GameType> kinds) {
        ...
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Rules.Fields.createdBy).is(userId).and(Rules.Fields.kind).in(kinds));
    ...
    public List<RulesSummary> listRulesOfKind(String userId, GameType kind) {
        MatchOperation matchOperation = Aggregation.match(Criteria.where(Rules.Fields.createdBy).is(userId).and(Rules.Fields.kind).is(kind));
        SortOperation sortOperation = Aggregation.sort(Sort.Direction.ASC, RulesSummary.Fields.name);
        return mongoTemplate.aggregate(Aggregation.newAggregation(matchOperation, sRulesSummaryProjection, sortOperation),
                mongoTemplate.getCollectionName(Rules.class), RulesSummary.class).getMappedResults();
    }
    ...
    public void updateScheduledGamesWithRules(String userId, Rules rules) {
        Query query = Query.query(
                Criteria.where(Game.Fields.createdBy).is(userId)
                        ...
                        .and(Game.Fields.rules + "." + _id).is(rules.getId())
                        .and(Game.Fields.rules + "." + Rules.Fields.createdBy).is(userId));
    ...
    public boolean existsByCreatedByAndNameAndKind(String userId, String name, GameType kind) {
        Query query = Query.query(Criteria.where(Rules.Fields.createdBy).is(userId).and(Rules.Fields.name).is(name).and(Rules.Fields.kind).is(kind));
        return mongoTemplate.exists(query, Rules.class);
    }
