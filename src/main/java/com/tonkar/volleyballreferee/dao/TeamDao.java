...
public class TeamDao {
    ...
    private final static ProjectionOperation sTeamSummaryProjection = Aggregation.project()
            .and(_id).as(_id)
            .and(Team.Fields.createdBy).as(TeamSummary.Fields.createdBy)
            .and(Team.Fields.createdAt).as(TeamSummary.Fields.createdAt)
            .and(Team.Fields.updatedAt).as(TeamSummary.Fields.updatedAt)
            .and(Team.Fields.name).as(TeamSummary.Fields.name)
            .and(Team.Fields.kind).as(TeamSummary.Fields.kind)
            .and(Team.Fields.gender).as(TeamSummary.Fields.gender);
    ...
    public Page<TeamSummary> listTeams(String userId, List<GameType> kinds, List<GenderType> genders, Pageable pageable) {
        ...
        Criteria criteria = Criteria.where(Team.Fields.createdBy).is(userId).and(Team.Fields.kind).in(kinds).and(Team.Fields.gender).in(genders);
    ...
    public void updateScheduledGamesWithHomeTeam(String userId, Team team) {
        Query query = Query.query(
                Criteria.where(Game.Fields.createdBy).is(userId)
                        .and(Game.Fields.status).is(GameStatus.SCHEDULED)
                        .and(Game.Fields.homeTeam + "." + _id).is(team.getId())
                        .and(Game.Fields.homeTeam + "." + Team.Fields.createdBy).is(userId));
    ...
    public void updateScheduledGamesWithGuestTeam(String userId, Team team) {
        Query query = Query.query(
                Criteria.where(Game.Fields.createdBy).is(userId)
                        .and(Game.Fields.status).is(GameStatus.SCHEDULED)
                        .and(Game.Fields.guestTeam + "." + _id).is(team.getId())
                        .and(Game.Fields.guestTeam + "." + Team.Fields.createdBy).is(userId));
    ...
    public boolean existsByCreatedByAndNameAndKindAndGender(String userId, String name, GameType kind, GenderType gender) {
        Query query = Query.query(Criteria.where(Team.Fields.createdBy).is(userId).and(Team.Fields.name).is(name).and(Team.Fields.kind).is(kind).and(Team.Fields.gender).is(gender));
        return mongoTemplate.exists(query, Team.class);
    }
